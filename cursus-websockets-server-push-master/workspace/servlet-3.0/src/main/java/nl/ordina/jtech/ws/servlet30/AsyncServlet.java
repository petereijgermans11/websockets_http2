...
public class AsyncServlet extends HttpServlet {
	...
	public void service(final ServletRequest req, final ServletResponse res) throws ServletException, IOException {
		...
		ctx.addListener(new AsyncListener() {
			...
			/**
			 * THIS NEVER GETS CALLED - error has occurred in async task...
			 * handle it
			 */
			@Override
		...
		executorService.execute(new Runnable() {
			...
			public void run() {
				try {
					...
					// note: request & response are not threadsafe
					final ServletRequest request = ctx.getRequest();
					final String someParam = request.getParameter("someParam");
